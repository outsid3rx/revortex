...
Для запуска приложения можно использовать параметры командной строки, которые будут переопределять значения в конфигурационном файле `revortex.json`.

```shell
pnpm exec revortex --out ./lib/ --alias ~server/ --mainPath ./src/main.ts --sourceDir src/ ../server/
```

### Конфигурация через файл или package.json

Создайте файл `revortex.json` в корне проекта или создайте поле `revortex` в `package.json` и установите необходимые поля:
...
Если вы хотите явно указать имя конечного файла, в который будет сохранен клиент, можно использовать этот параметр. По умолчанию: `index.ts`.
